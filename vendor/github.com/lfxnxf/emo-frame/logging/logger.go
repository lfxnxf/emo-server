...
import (
	...
	"context"
	"fmt"
	"github.com/lfxnxf/emo-frame/rolling"
...
func init() {
	...
	if host, _ := utils.HostIP(); host != nil {
...
func RemoveExpireLogs(path string, storageDay int64) {
	...
	for {
		...
		for _, f := range files {
			...
			if err != nil {
				_defaultLogger.Errorw("os.Open error", zap.Error(err))
				continue
			}
			fi, err := f.Stat()
			if err != nil {
			...
			if fi.ModTime().Unix() > time.Now().Unix()-int64(expireDayTime) {
				continue
			}
			// 删除文件
